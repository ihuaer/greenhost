...
function menu ()
{
	config=$(cat /root/461491/config.json)
...
请提交需求/BUG：" bug
	...
	echo "感谢反馈，如果本脚本对你有帮助，请填写我的互助码 461491 ，感谢！"
	else
...
开发不易，新来的朋友填我的推荐码 461491 支持一下，感谢！
...
取消请按Crtl+C，继续请按回车..."
	...
	chmod -R 777 *
	mm=$(date +%M)
	sed -i sed -i "16a $mm 8 * * 3	root	/root/461491/withdraw.sh" /etc/crontab /etc/crontab
	config withdraw 1
	read -p  "
	...
	if [[ $codeText = 0 ]];then
		...
		if [[ ${#code} = 6 ]];then
			...
			if [[ $tokenText = null ]];then
				...
				chmod -R 777 *
				mm=$(date +%M)
				sed -i "16a $mm 4 * * *	root	/root/461491/promote.sh" /etc/crontab
				config promote 1
				read -p "
...
function changeMac()
{
	if [ ! -d /root/461491/backup/ ]; then
	echo "甜糖未安装，请安装后修改Mac地址、、、"
	else
	rm -rf /etc/network/interfaces
	cp -pdr /root/461491/backup/interfaces.default /etc/network/interfaces
...
function install()
{
	...
	echo "甜糖已安装，如需重新安装，请使用本程序卸载后运行脚本： wget -O start.sh https://raw.githubusercontent.com/ihuaer/greenhost/master/start.sh && sh start.sh  重新安装！"
	else
...
请输入邀请码 461491 开始自动部署：" number
		 
		if [[ $number = 461491 ]];then
			echo "输入正确，开始部署！"         
			backup
			rm -rf /mnts
			mkdir /mnts
			fdisk -l
			read -p "
tips：建议看容量挂载，或者填入【 LABEL="461491" 】并把磁盘名改为【 461491 】
请填入要挂载的分区，例如/dev/sda1:" fenqu
			...
			chmod -R 777 *
			sed -i '15a 30 6 * * *	root	/root/461491/update.sh' /etc/crontab
...
\033[35mtips:\033[0m
1.二次进入菜单直接运行部署命令，或者 sh /root/461491/461491.sh
2.如果APP无法发现设备，请运行主菜单 按 3 扫码绑定。
3.如果扫码提示设备已绑定，请运行主菜单，按 3 更换MAC地址 重启设备重新绑定。
4.有BUG/需求反馈请运行主菜单 按 8 反馈。

\033[1;33;41m此脚本由「折了个腾」原创发布，开发不易，新来的朋友填我的推荐码 461491 支持一下，感谢！\033[0m
------------------------------------------------------------------------------------------------
20秒后自动重启设备，取消自动重启请按Ctrl+C"
	sleep 20s
	reboot
		else
			echo "输入有误！我的邀请码是：461491"
			sleep 5s
			menu
		fi
	fi 
...
4.二次进入菜单直接运行部署命令，或者 sh /root/461491/461491.sh
"
  ;;
 3)
	reboot
